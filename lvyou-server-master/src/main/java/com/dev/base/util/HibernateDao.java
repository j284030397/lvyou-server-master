...
//			if (queryObject.getPageObj()!=null) {
...
//					}
//				});
//				queryObject.getPageObj().setTotalPageCount(total);
//				queryObject.setStart(queryObject.getPageObj().getStart());
//				queryObject.setLimit(queryObject.getPageObj().getPageSize());
	      ...
	      public Object extractData(ResultSet rs) throws SQLException, DataAccessException
	      {
	        ...
	        if (rs.next()) {
	          ...
	          do
	          {
	            Map map = new HashMap();
	            for (int i = 0; i < fields_nums; i++) {
	              map.put(fields[i], HibernateDao.this.addResultData(rs, fields[i], 
	                sqlTypes[i], i).trim());
	            }
	            list.add(map);
	          }

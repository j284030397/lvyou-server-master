...
@Repository("LyUserFollowDao")
public class LyUserFollowDaoImpl extends BaseLocationDaoImpl implements LyUserFollowDao {
	 private static final String TABLE_STR = LyUserFollow.T_NAME; 
	    private static final String CLASS_STR = LyUserFollow.class.getSimpleName();
		...
		public Collection<LyUserFollow> queryByField(String fieldName, Object fieldValue)throws DAOException {
	    	StringBuffer hql = new StringBuffer().append("from ").append(CLASS_STR).append(" a where a.")
				 								 .append(fieldName).append("=:fieldValue");
	    	Collection<LyUserFollow> resutlList = getHibernateTemplate().findByNamedParam(hql.toString(), "fieldValue" , fieldValue) ;
			return resutlList;
		}
	    ...
	    public Collection<LyUserFollow> queryByFields(String[] fieldNames, String[] logicOpers, Object[] fieldValues) throws DAOException {
	    	...
	    	Collection<LyUserFollow> resutlList = getHibernateTemplate().findByNamedParam(hql.toString(), fieldNames, fieldValues) ; 
	    	return resutlList;
	    }

		public QueryObject query(QueryObject queryObject) throws DAOException {
			StringBuffer fields = new StringBuffer(64) ;
			fields.append(LyUserFollow.C_SID).append(" as ").append(underscoreName(LyUserFollow.C_SID)).append(",") ;
			fields.append(LyUserFollow.C_USERNAME).append(" as ").append(underscoreName(LyUserFollow.C_USERNAME)).append(",") ;
			fields.append(LyUserFollow.C_FOLLOWUSERNAME).append(" as ").append(underscoreName(LyUserFollow.C_FOLLOWUSERNAME)).append(",") ;
			fields.append(LyUserFollow.C_POS).append(" as ").append(underscoreName(LyUserFollow.C_POS)).append(",") ;
			fields.append(LyUserFollow.C_CTIME).append(" as ").append(underscoreName(LyUserFollow.C_CTIME)).append(",") ;
			fields.append(LyUserFollow.C_MEMO).append(" as ").append(underscoreName(LyUserFollow.C_MEMO)).append(",") ;
			if(fields.length() > 0){

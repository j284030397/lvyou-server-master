...
@Repository("LyWantToDao")
public class LyWantToDaoImpl extends BaseLocationDaoImpl implements LyWantToDao {
	 private static final String TABLE_STR = LyWantTo.T_NAME; 
	    private static final String CLASS_STR = LyWantTo.class.getSimpleName();
		...
		public Collection<LyWantTo> queryByField(String fieldName, Object fieldValue)throws DAOException {
	    	StringBuffer hql = new StringBuffer().append("from ").append(CLASS_STR).append(" a where a.")
				 								 .append(fieldName).append("=:fieldValue");
	    	Collection<LyWantTo> resutlList = getHibernateTemplate().findByNamedParam(hql.toString(), "fieldValue" , fieldValue) ;
			return resutlList;
		}
	    ...
	    public Collection<LyWantTo> queryByFields(String[] fieldNames, String[] logicOpers, Object[] fieldValues) throws DAOException {
	    	...
	    	Collection<LyWantTo> resutlList = getHibernateTemplate().findByNamedParam(hql.toString(), fieldNames, fieldValues) ; 
	    	return resutlList;
	    }

		public QueryObject query(QueryObject queryObject) throws DAOException {
			StringBuffer fields = new StringBuffer(64) ;
			fields.append(LyWantTo.C_SID).append(" as ").append(underscoreName(LyWantTo.C_SID)).append(",") ;
			fields.append(LyWantTo.C_USERNAME).append(" as ").append(underscoreName(LyWantTo.C_USERNAME)).append(",") ;
			fields.append(LyWantTo.C_SPOTSID).append(" as ").append(underscoreName(LyWantTo.C_SPOTSID)).append(",") ;
			fields.append(LyWantTo.C_TOTALPEOPLEBYCITY).append(" as ").append(underscoreName(LyWantTo.C_TOTALPEOPLEBYCITY)).append(",") ;
			fields.append(LyWantTo.C_POS).append(" as ").append(underscoreName(LyWantTo.C_POS)).append(",") ;
			fields.append(LyWantTo.C_CTIME).append(" as ").append(underscoreName(LyWantTo.C_CTIME)).append(",") ;
			fields.append(LyWantTo.C_MEMO).append(" as ").append(underscoreName(LyWantTo.C_MEMO)).append(",") ;
			if(fields.length() > 0){

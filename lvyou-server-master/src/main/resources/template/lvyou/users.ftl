...
import com.dev.scan.dao.BaseLocationDaoImpl;
import com.dev.scan.dao.${tableClassName}Dao;
import com.dev.scan.model.${tableClassName};


@Repository("${tableClassName}Dao")
public class ${tableClassName}DaoImpl extends BaseLocationDaoImpl implements ${tableClassName}Dao {
	 private static final String TABLE_STR = ${tableClassName}.T_NAME; 
	    private static final String CLASS_STR = ${tableClassName}.class.getSimpleName();
		...
		public Collection<${tableClassName}> queryByField(String fieldName, Object fieldValue)throws DAOException {
	    	StringBuffer hql = new StringBuffer().append("from ").append(CLASS_STR).append(" a where a.")
				 								 .append(fieldName).append("=:fieldValue");
	    	Collection<${tableClassName}> resutlList = getHibernateTemplate().findByNamedParam(hql.toString(), "fieldValue" , fieldValue) ;
			return resutlList;
		}
	    ...
	    public Collection<${tableClassName}> queryByFields(String[] fieldNames, String[] logicOpers, Object[] fieldValues) throws DAOException {
	    	...
	    	Collection<${tableClassName}> resutlList = getHibernateTemplate().findByNamedParam(hql.toString(), fieldNames, fieldValues) ; 
	    	return resutlList;
	    }
		...
		public QueryObject query(QueryObject queryObject) throws DAOException {
			...
			fields.append(${tableClassName}.C_${columnbean.colName?upper_case}).append(" as ").append(underscoreName(${tableClassName}.C_${columnbean.colName?upper_case})).append(",") ;
			</#list>
			if(fields.length() > 0){
